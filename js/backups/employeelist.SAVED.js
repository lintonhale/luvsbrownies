...
//	tx.executeSql(sql, [], getEmployees_success);
	...
	var sql2 = "select sg.id, sg.foodsubgroup " + 
			"from foodsubgroups sg order by sg.id";
	tx.executeSql(sql2, [], getFoodSubgroups_success);
	
	var sql3 = "select s.id, s.foodstandard " + 
			"from foodstandards s order by s.id";
	tx.executeSql(sql3, [], getFoodStandards_success);
	
	var sql4 = "select t.id, t.foodtype " + 
			"from foodtypes t order by t.id";	
	tx.executeSql(sql4, [], getFoodTypes_success);

	var sql5 = "select i.id, i.foodtype, i.foodgroup, i.foodsubgroup " + 
			"from fooditems i order by i.id";	
	tx.executeSql(sql5, [], getFoodItems_success);
...
function getFoodSubgroups_success(tx, results) {
    ...
    for (var i=0; i<len; i++) {
		...
		$('#employeeList').append('<li><a href="productdetails.html?id=' + foodsubgroups.id + '">' +
				'<p class="line1">' + foodsubgroups.foodgroup + '</p>' +
				'<p class="line2">' + foodsubgroups.foodsubgroup + '</p>' +
